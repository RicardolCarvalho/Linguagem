...
// Função para habilitar/desabilitar geração de código LLVM (definida em parser.y)
extern void enable_llvm_codegen(int enable);
...
int main(int argc, char **argv) {
    ...
    ExecutionMode exec_mode = MODE_INTERPRET;
    ...
    // Habilitar geração de código LLVM se necessário
    if (exec_mode == MODE_LLVM_IR || exec_mode == MODE_LLVM_EXEC) {
        enable_llvm_codegen(1);
    }
